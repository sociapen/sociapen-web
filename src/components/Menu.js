			...
			return (
				<div className='sociapen-App-menu-interaction-pane'>
					<div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
						...
						<h1 style={{ color: 'var(--fore-color)' }}>{currentUserAdditionalData.name}</h1>
					</div>
					<div className='sociapen-App-menu-interaction-pane-profile-info'>
						<div style={{ display: 'flex', alignItems: 'flex-end' }}><img src={SVGperson} alt="username" />&nbsp;<span>Username</span>&nbsp;<span style={{ color: '#000000' }}>{currentUserAdditionalData.username}</span></div>
						<div style={{ display: 'flex', alignItems: 'flex-end' }}><img src={SVGemail} alt="email" />&nbsp;<span>Email</span>&nbsp;<span style={{ color: '#000000' }}>{currentUser.email}</span></div>
						{currentUser.phoneNumber ? <div style={{ display: 'flex', alignItems: 'flex-end' }}><img src={SVGphone} alt="phone" />&nbsp;<span style={{ color: '#000000' }}>Phone</span>&nbsp;<span>{currentUser.phoneNumber}</span></div> : null}
						<div style={{ display: 'flex', alignItems: 'flex-end' }}><img src={SVGcalender} alt="joined on" />&nbsp;<span>Account Created On</span>&nbsp;<span style={{ color: '#000000' }}>{new Date(currentUser.metadata.creationTime ?? Date.now()).toDateString()}</span></div>
						<div style={{ display: 'flex', alignItems: 'flex-end' }}><img src={SVGcalender} alt="last login" />&nbsp;<span>Last SignIn On</span>&nbsp;<span style={{ color: '#000000' }}>{new Date(currentUser.metadata.lastSignInTime ?? Date.now()).toDateString()}</span></div>
					</div>
